...
func (s *TestHookSuite) Test_NewPublisher_DefaultsTgSenderToBot() {
	bot := &tgbotapi.BotAPI{}
	p := NewNotificationPublisher(bot, s.dbConn)

	sender, ok := p.tgSender.(*tgbotapi.BotAPI)
	require.True(s.T(), ok)
	require.Same(s.T(), bot, sender)
}
